...
import { Activity, Shield, User as UserIcon, Lock, ArrowRight, CheckCircle2, Sun, Moon } from 'lucide-react';
  ...
  return (
    <div className={`relative min-h-screen flex items-center justify-center overflow-hidden font-sans radial-glow grid-bg px-4 transition-colors duration-300 ${
        ...
        <button
          ...
          className={`h-9 w-9 rounded-lg border flex items-center justify-center cursor-pointer transition-all shadow-sm ${
            darkMode 
              ...
              : 'border-slate-200 bg-white text-indigo-650 hover:bg-slate-100'
          }`}
        >
      ...
      <div className="w-full max-w-md z-10">
        ...
        {/* Login Form Panel */}
        <div className={`rounded-2xl border shadow-2xl p-8 mb-6 animate-in fade-in duration-300 ${
          darkMode ? 'border-slate-800 bg-slate-900/50 text-white' : 'border-slate-200 bg-white text-slate-900'
        }`}>
          <h2 className="text-xl font-bold mb-6">Workspace Sign In</h2>
        ...
        {showSeedTips && (
          <div className={`p-5 rounded-2xl border backdrop-blur-md animate-in slide-in-from-bottom-6 duration-300 ${
            darkMode ? 'border-teal-900/30 bg-teal-950/20' : 'border-teal-200/50 bg-teal-50/50'
          }`}>
            ...
            <div className="grid grid-cols-2 gap-3.5">
              <button
                ...
                className={`p-3 text-left rounded-xl border transition-all text-xs cursor-pointer group ${
                  darkMode 
                    ? 'bg-slate-900/70 border-slate-800 hover:border-teal-500/50 hover:bg-slate-900' 
                    : 'bg-white border-slate-200 hover:border-teal-500/50 hover:bg-slate-50 shadow-sm'
                }`}
              >
                <div className={`font-bold flex items-center gap-1 ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                  <CheckCircle2 size={12} className="text-teal-500" />
                  Admin Account
                </div>
                <div className="text-[10px] text-slate-450 mt-1 font-mono">admin@mindmesh.com</div>
              </button>
              ...
              >
                ...
                <div className="text-[10px] text-slate-450 mt-1 font-mono">member@mindmesh.com</div>
              </button>
            </div>
            <div className="text-[10px] text-teal-650 mt-2.5 text-center font-mono">
